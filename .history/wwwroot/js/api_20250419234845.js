    ...
    getGames: async function (filters = {}) {
        try {
            ...
            if (filters.genreId) {
                ...
                // Get genre name from the filter ID
                const genreSelect = document.getElementById('genre-filter');
                let genreName = null;

                if (genreSelect) {
                    for (let i = 0; i < genreSelect.options.length; i++) {
                        if (genreSelect.options[i].value === String(filters.genreId)) {
                            genreName = genreSelect.options[i].textContent;
                            break;
                        }
                    }
                }

                // Fallback to a hardcoded genre name mapping if needed
                if (!genreName) {
                    const genreMap = {
                        '1': 'Fighting',
                        '2': 'Racing',
                    ...
                    genreName = genreMap[filters.genreId] || null;
                }

                console.log('Looking for genre name:', genreName);

                games = games.filter(game => {
                    console.log(`Game ${game.name} - genre: ${game.genre}, looking for: ${genreName}`);
                    return game.genre === genreName;
                });
