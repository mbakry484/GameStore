document.addEventListener('DOMContentLoaded', function () {
    ...
    async function uploadImage(file) {
        // Simple file upload using FormData
        return new Promise((resolve, reject) => {
            const fileName = `${Date.now()}_${file.name.replace(/\s+/g, '_')}`;
            const relativePath = `/images/games/${fileName}`;
            
            // Simulate file upload by using the FileReader to read the file
            // In a real app, you'd upload to the server using fetch/AJAX
            const reader = new FileReader();
            
            reader.onload = function(e) {
                try {
                    // In a real app, you'd handle the upload to server
                    // For now, we'll simulate a successful upload by returning the path
                    
                    // Create image element to test if image can be loaded
                    const img = new Image();
                    img.onload = function() {
                    ...
                    // For demo purposes, let's store the image data in localStorage
                    // In a production app, you'd upload to server instead
                    localStorage.setItem(`gameImage_${fileName}`, e.target.result);
                } catch (error) {
                    reject(error);
                }
            };
