...
const api = {
    ...
    getGames: async function (filters = {}) {
        try {
            ...
            if (filters.genreId) {
                console.log('Filtering by genre ID:', filters.genreId);
                console.log('Games before filtering:', games.length);
                console.log('Sample game data:', games.length > 0 ? JSON.stringify(games[0]) : 'No games');

                games = games.filter(game => {
                    // Look for genreID in various possible formats/properties
                    const gameGenreId = game.genreID || game.genreId ||
                        (game.genre ? (typeof game.genre === 'object' ? game.genre.id : null) : null);

                    console.log(`Game ${game.name} - Full game data:`, JSON.stringify(game));
                    return gameGenreId == parseInt(filters.genreId) ||
                        (game.genre && game.genre === filters.genreId) ||
                        (game.genre && typeof game.genre === 'string' &&
                            document.querySelector(`#genre-filter option[value="${filters.genreId}"]`)?.textContent === game.genre);
                });
            ...
            return games.map(game => ({
                id: game.id,
                name: game.name,
                genreID: game.genreID || game.genreId || 0,
